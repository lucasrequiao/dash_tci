...
aba_geral, aba_historico = st.tabs(["📊 Visão Geral", "🔍 Histórico por Estabelecimento"])
...
with aba_historico:
    ...
    if escolha_historico:
        chave_sel = opcoes[escolha_historico]
        historico = df_filtrado[df_filtrado["chave_estabelecimento"] == chave_sel].sort_values("dataCadastro", ascending=False)
        
        h1, h2, h3 = st.columns(3)
        h1.metric("Total de TCIs", len(historico))
        h2.metric("1º TCI", historico["dataCadastro"].min().strftime("%d/%m/%Y"))
        h3.metric("Último TCI", historico["dataCadastro"].max().strftime("%d/%m/%Y"))
